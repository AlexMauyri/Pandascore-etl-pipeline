...
from common.settings import ClickHouseSettings, get_clickhouse_settings
 
setup_logging()
logger = logging.getLogger(__name__)
 
ENTITIES_WITHOUT_ID = {"match_opponents"}
...
class ClickHouseLoader:
    ...
    def __init__(self, settings: ClickHouseSettings):
        logger.info(f"Подключение к ClickHouse: {settings.host}:{settings.port}/{settings.db}")
        ...
        self.insert_chunk_size = settings.insert_chunk_size
    ...
    def _insert_batch(self, table_name: str, dicts: List[Dict]) -> int:
        ...
        for offset in range(0, len(dicts), self.insert_chunk_size):
            chunk = dicts[offset : offset + self.insert_chunk_size]
            data = [[convert_value(row.get(col), col) for col in column_names] for row in chunk]
 
            logger.info(
                f"[{table_name}] Загрузка чанка {offset // self.insert_chunk_size + 1} "
                f"({len(data)} записей), колонки: {column_names}"
            )
            self.client.insert(table_name, data, column_names=column_names)
            total_inserted += len(data)
 
        logger.info(f"[{table_name}] Успешно загружено {total_inserted} записей")
    ...
    def load_entities(self, entities: Dict[str, List]) -> Dict[str, int]:
        ...
        logger.info(f"Загрузка завершена. Всего записей: {total_loaded}. По таблицам: {loaded_counts}")
    ...
    def log_run(
        ...
        logger.info(
            ...
            f"status={status}, extracted={rows_extracted}, loaded={rows_loaded}"
        )
...
def _update_state_for_entities(loader: ClickHouseLoader, entities: Dict[str, List]) -> None:
    for entity_name, models in entities.items():
        if not models or entity_name in ENTITIES_WITHOUT_ID:
            continue
 
        try:
            last_id = max(getattr(m, "id") for m in models)
        except AttributeError:
            logger.warning(
                f"[{entity_name}] Модель не содержит поле id, "
                f"пропускаем обновление etl_state (добавьте в ENTITIES_WITHOUT_ID, "
                f"если это ожидаемо)"
            )
            continue
 
        if last_id:
            loader.update_state(entity_name=entity_name, last_id=last_id)
    ...
    try:
        ...
        if loader is None:
            try:
                ...
                logger.error(f"Не удалось подключиться для записи etl_run_log: {conn_err}")
                loader = None
...
if __name__ == "__main__":
    ...
    load_to_clickhouse(entities=objects)
